...
class CLI extends \WP_CLI_Command {
	...
	public function backup( $args, $assoc_args ) {
		...
		add_action( 'hmbkp_mysqldump_started', function () {
			\WP_CLI::line( __( 'Backup: Dumping database...', 'backup-restore-manager' ) );
		} );

		add_action( 'hmbkp_archive_started', function () {
			\WP_CLI::line( __( 'Backup: Zipping everything up...', 'backup-restore-manager' ) );
		} );
		...
		if ( ( ! is_dir( Path::get_path() ) ) ) {
			\WP_CLI::error( __( 'Invalid backup path', 'backup-restore-manager' ) );
			return false;
		}

		if ( ! is_dir( Path::get_root() ) || ! is_readable( Path::get_root() ) ) {
			\WP_CLI::error( __( 'Invalid root path', 'backup-restore-manager' ) );
			return false;
		}
		...
		if ( file_exists( $hm_backup->get_backup_filepath() ) ) {
			\WP_CLI::success( __( 'Backup Complete: ', 'backup-restore-manager' ) . $hm_backup->get_backup_filepath() );
		} else {
			\WP_CLI::error( __( 'Backup Failed', 'backup-restore-manager' ) );
		}
