...
class Scheduled_Backup {
	...
	public function set_reoccurrence( $reoccurrence ) {
		...
		if ( ! is_string( $reoccurrence ) || ! trim( $reoccurrence ) || ( ! in_array( $reoccurrence, array_keys( $hmbkp_schedules ) ) ) && 'manually' !== $reoccurrence ) {
			return new \WP_Error( 'hmbkp_invalid_argument_error', sprintf( __( 'Argument 1 for %s must be a valid cron recurrence or "manually"', 'backup-restore-manager' ), __METHOD__ ) );
		}
	...
	public function run() {
		...
		$this->status->start( $this->get_backup_filename(), __( 'Starting backup...', 'backup-restore-manager' ) );

		$this->status->set_status( __( 'Deleting old backups...', 'backup-restore-manager' ) );
	...
	public function get_schedule_average_duration() {
		...
		if ( $average_run_time < HOUR_IN_SECONDS ) {
			...
			$duration = sprintf( _n( '%s min', '%s mins', $mins, 'backup-restore-manager' ), $mins );
			...
			$duration = sprintf( _n( '%s hour', '%s hours', $hours, 'backup-restore-manager' ), $hours );
		}
	...
	public function delete_backup( $filepath ) {
		...
		if ( empty( $filepath ) || ! is_string( $filepath ) ) {
			return new \WP_Error( 'hmbkp_empty_string_error', sprintf( __( 'Argument 1 for %s must be a non-empty string', 'backup-restore-manager' ), __METHOD__ ) );
		}

		// Make sure it exists
		if ( ! file_exists( $filepath ) ) {
			return new \WP_Error( 'hmbkp_file_error', sprintf( __( '%s doesn\'t exist', 'backup-restore-manager' ), $filepath ) );
		}

		// Make sure it was created by this schedule
		if ( strpos( $filepath, $this->get_id() ) === false ) {
			return new \WP_Error( 'hmbkp_backup_error', __( 'That backup wasn\'t created by this schedule', 'backup-restore-manager' ) );
		}
