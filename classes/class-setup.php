...
class HMBKP_Setup {
	...
	public static function activate() {
		...
		load_plugin_textdomain( 'backup-restore-manager', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );

		if ( ! self::meets_requirements() ) {

			wp_die( self::get_notice_message(), __( 'Backup & Restore Manager', 'backup-restore-manager' ), array( 'back_link' => true ) );
	...
	/**
	 * Deactivate Backup & Restore Manager.
	 */
	public static function self_deactivate() {
		...
		deactivate_plugins( dirname( dirname( __FILE__ ) ) . '/backup-restore-manager.php' );
	...
	public static function get_notice_message() {
		...
		*/
		return sprintf(
			__( 'Backup & Restore Manager requires PHP version %1$s or later and WordPress version %2$s or later to run. It has not been activated.', 'backup-restore-manager' ),
			self::MIN_PHP_VERSION,
			self::MIN_WP_VERSION
		);
		/* OBZMOD */
