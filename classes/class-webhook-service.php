...
abstract class Webhook_Service extends Service {
	...
	public function action( $action, Backup $backup ) {
		...
		if ( ! file_exists( $file ) && ( $errors = array_merge( $backup->get_errors(), $backup->get_warnings() ) ) ) {
			...
			$subject = sprintf( __( 'Backup of %s Failed', 'backup-restore-manager' ), $domain );
		...
		if ( is_wp_error( $ret ) ) {
			$backup->warning( 'Webhook', sprintf( __( 'Error: %s', 'backup-restore-manager' ), $ret->get_error_message() ) );
		}
