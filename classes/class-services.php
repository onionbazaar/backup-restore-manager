...
class Services {
	...
	public static function register( $filepath, $classname ) {
		if ( ! file_exists( $filepath ) ) {
			return new \WP_Error( 'hmbkp_invalid_path_error', sprintf( __( 'Argument 1 for %s must be a valid filepath', 'backup-restore-manager' ), __METHOD__ ) );
		}
	...
	public static function unregister( $filepath ) {
		...
		if ( ! isset( self::instance()->services[ $filepath ] ) ) {
			return new \WP_Error( 'hmbkp_unrecognized_service_error', sprintf( __( 'Argument 1 for %s must be a registered service', 'backup-restore-manager' ), __METHOD__ ) );
		}
	...
	private static function instantiate( $classname ) {
		...
		if ( ! class_exists( $classname ) ) {
			return new \WP_Error( 'hmbkp_invalid_type_error', sprintf( __( 'Argument 1 for %s must be a valid class', 'backup-restore-manager' ), __METHOD__ ) );
		}
