...
final class Plugin {
	...
	*/
	...
	private function __construct() {
		...
		*/
		$hide_notice = get_site_option( 'hmbkp_hide_info_notice', true );
		/* OBZMOD */
	...
	public function plugins_loaded() {
		...
		if ( true !== $this->maybe_self_deactivate() ) {
			...
			do_action( 'backup-restore-manager_loaded' );
	...
	/**
	 * Load all Backup & Restore Manager functions.
	 */
	...
	/**
	 * Hook into WordPress page lifecycle and execute Backup & Restore Manager functions.
	 */
	...
	public function scripts( $hook ) {
		...
		wp_localize_script(
			...
			array(
				...
				'update'                   => __( 'Update', 'backup-restore-manager' ),
				'cancel'                   => __( 'Cancel', 'backup-restore-manager' ),
				'delete_schedule'          => __( 'Are you sure you want to delete this schedule? All of its backups will also be deleted.', 'backup-restore-manager' ) . "\n\n" . __( '\'Cancel\' to go back, \'OK\' to delete.', 'backup-restore-manager' ) . "\n",
				'delete_backup'            => __( 'Are you sure you want to delete this backup?', 'backup-restore-manager' ) . "\n\n" . __( '\'Cancel\' to go back, \'OK\' to delete.', 'backup-restore-manager' ) . "\n",
				'remove_exclude_rule'      => __( 'Are you sure you want to remove this exclude rule?', 'backup-restore-manager' ) . "\n\n" . __( '\'Cancel\' to go back, \'OK\' to delete.', 'backup-restore-manager' ) . "\n",
				'remove_old_backups'       => __( 'Reducing the number of backups that are stored on this server will cause some of your existing backups to be deleted. Are you sure that\'s what you want?', 'backup-restore-manager' ) . "\n\n" . __( '\'Cancel\' to go back, \'OK\' to delete.', 'backup-restore-manager' ) . "\n",
			)
		);
	 ...
	 * Files will need to be in a subdirectory with the name of the textdomain 'backup-restore-manager'
	 */
	public function text_domain() {

		// Set unique textdomain string
		$textdomain = 'backup-restore-manager';
	...
	/**
	 * Ensure Backup & Restore Manager is loaded before add-ons, changes the order of the serialized values in the DB field.
	 */
	...
	public function styles( $hook ) {
		...
		if ( 'tools_page_backup-restore-manager_extensions' !== $hook && HMBKP_ADMIN_PAGE !== $hook ) {
			return;
		}
		...
		wp_enqueue_style( 'backup-restore-manager', $css_file, false, sanitize_key( self::PLUGIN_VERSION ) );
	...
	public function display_feature_message() {
		...
		*/
		$info_message = __( 'Thanks for updating Backup & Restore Manager.', 'backup-restore-manager' );
		/* OBZMOD */
		?>
		...
		<div id="hmbkp-info-message" class="updated notice is-dismissible">
			...
			<button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'backup-restore-manager' ); ?></span></button>
