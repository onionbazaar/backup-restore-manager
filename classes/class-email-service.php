...
class Email_Service extends Service {
	...
	?>
		...
		<tr>
			...
			<th scope="row">
				<label for="<?php echo esc_attr( $this->get_field_name( 'email' ) ); ?>"><?php _e( 'Email notification', 'backup-restore-manager' ); ?></label>
			</th>
			...
			<td>
				...
				<p class="description"><?php printf( __( 'Receive a notification email when a backup completes. If the backup is small enough (&lt; %s), then it will be attached to the email. Separate multiple email addresses with a comma.', 'backup-restore-manager' ), '<code>' . size_format( get_max_attachment_size() ) . '</code>' ); ?></p>
			</td>
	...
	?>
		...
		<tr<?php if ( defined( 'HMBKP_ATTACHMENT_MAX_FILESIZE' ) ) { ?> class="hmbkp_active"<?php } ?>>
			...
			<td>
				...
				<p><?php printf( __( 'You\'ve set it to: %s', 'backup-restore-manager' ), '<code>' . HMBKP_ATTACHMENT_MAX_FILESIZE . '</code>' ); ?></p>
				<?php } ?>

				<p><?php printf( __( 'The maximum filesize of your backup that will be attached to your notification emails . Defaults to %s.', 'backup-restore-manager' ), '<code>10MB</code>' ); ?> <?php _e( 'e.g.', 'backup-restore-manager' ); ?> <code>define( 'HMBKP_ATTACHMENT_MAX_FILESIZE', '25MB' );</code></p>
	...
	public function display() {
		...
		if ( $emails = $this->get_email_address_array() ) {
			...
			return sprintf( __( 'Send an email notification to %s', 'backup-restore-manager' ), $email );
		...
		if ( isset( $new_data['email'] ) ) {
			...
			if ( ! empty( $new_data['email'] ) ) {
				...
				foreach ( explode( ',', $new_data['email'] ) as $email ) {
					...
					if ( ! is_email( $email ) ) {
						$errors['email'] = sprintf( __( '%s isn\'t a valid email',  'backup-restore-manager' ), $email );
					}
				}
			}
	...
	public function action( $action, Backup $backup ) {
		...
		if ( 'hmbkp_backup_complete' === $action && $this->get_email_address_array() ) {
			...
			$headers  = 'From: Backup & Restore Manager <' . apply_filters( 'hmbkp_from_email', get_bloginfo( 'admin_email' ) ) . '>' . "\r\n";
			...
			if ( ! file_exists( $file ) && ( $errors = array_merge( $backup->get_errors(), $backup->get_warnings() ) ) ) {
				...
				*/
				$message = sprintf( __( 'Backup & Restore Manager was unable to backup your site %1$s.', 'backup-restore-manager' ) . "\n\n" . __( 'Here are the errors that we\'ve encountered:', 'backup-restore-manager' ) . "\n\n" . '%2$s', home_url(), $error_message );
				/* OBZMOD */
			...
			if ( @filesize( $file ) < get_max_attachment_size() ) {
				...
				*/
				$message = sprintf( __( 'Backup & Restore Manager has completed a backup of your site %1$s.', 'backup-restore-manager' ) . "\n\n" . __( 'The backup file should be attached to this email.', 'backup-restore-manager' ) . "\n\n" . __( 'You can download the backup file by clicking the link below:', 'backup-restore-manager' ) . "\n\n" . '%2$s', home_url(),  $download );
				/* OBZMOD */
			...
			if ( ! $sent ) {
				...
				*/
				$message = sprintf( __( 'Backup & Restore Manager has completed a backup of your site %1$s.', 'backup-restore-manager' ) . "\n\n" . __( 'Unfortunately, the backup file was too large to attach to this email.', 'backup-restore-manager' ) . "\n\n" . __( 'You can download the backup file by clicking the link below:', 'backup-restore-manager' ) . "\n\n" . '%2$s', home_url(), $download );
				/* OBZMOD */
