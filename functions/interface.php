	...
	<tr class="hmbkp_manage_backups_row">
		...
		<td>
			...
			<?php if (  is_path_accessible( Path::get_path() )  ) : ?>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'hmbkp_backup_archive' => $encoded_file, 'hmbkp_schedule_id' => $schedule->get_id(), 'action' => 'hmbkp_request_download_backup' ), admin_url( 'admin-post.php' ) ), 'hmbkp_download_backup', 'hmbkp_download_backup_nonce' ) ); ?>" class="download-action"><?php _e( 'Download', 'backup-restore-manager' ); ?></a> |
			<?php endif; ?>

			<!-- OBZMOD -->
			<a href="<?php
			$hmbkp_nonce = wp_create_nonce( 'hmbkp_nonce' );
			...
			echo '?page=backup-restore-manager&_wpnonce=' . $hmbkp_nonce . $hmbkp_schedule_id_str . '&hmbkp_restore=' . basename($file);
			 ?>" class="download-action">
				<?php esc_html_e( 'Restore', 'backup-restore-manager'); ?>
			</a> |
			<!-- OBZMOD -->
			...
			<a href="<?php echo esc_url( wp_nonce_url(
				...
				<?php esc_html_e( 'Delete', 'backup-restore-manager' ); ?>
			</a>
...
function admin_notices() {
	...
	<?php if ( ! empty( $notices['backup_errors'] ) ) : ?>
		...
		<div id="hmbkp-warning-backup" class="error notice is-dismissible">
			<p>
				<strong><?php _e( 'Backup & Restore Manager detected issues with your last backup.', 'backup-restore-manager' ); ?></strong>
			</p>
...
function set_server_config_notices() {
	...
	if ( Backup_Utilities::is_safe_mode_on() ) {
		...
		$messages[] = sprintf(
			...
			__( '%1$s is running in <a href="http://php.net/manual/en/features.safe-mode.php">Safe Mode</a>, please contact your host and ask them to disable it. Backup & Restore Manager may not work correctly whilst <code>Safe Mode</code> is on.', 'backup-restore-manager' ),
			'<code>PHP</code>'
		);
	}
	...
	if ( defined( 'HMBKP_PATH' ) && HMBKP_PATH ) {
		...
		if ( ! path_in_php_open_basedir( HMBKP_PATH ) ) {
			...
			$messages[] = sprintf(
				__( 'Your server has an %1$s restriction in effect and your custom backups directory (%2$s) is not within the allowed path(s): (%3$s).', 'backup-restore-manager' ),
			...
			$messages[] = sprintf(
				__( 'Your custom backups directory (%1$s) doesn&apos;t exist, your backups will be saved to %2$s instead.', 'backup-restore-manager' ),
				'<code>' . esc_html( HMBKP_PATH ) . '</code>',
			...
			$messages[] = sprintf(
				__( 'Your custom backups directory (%1$s) isn&apos;t writable, new backups will be saved to %2$s instead.', 'backup-restore-manager' ),
				'<code>' . esc_html( HMBKP_PATH ) . '</code>',
				'<code>' . esc_html( Path::get_path() ) . '</code>'
			);
		}
	...
	if ( ! is_dir( Path::get_path() ) || is_dir( Path::get_path() ) && ! wp_is_writable( Path::get_path() ) ) {
		...
		if ( isset( $_GET['creation_error'] ) ) {
			...
			*/
			$messages[] = __( 'Connection successful but still not able to automatically create the directory. You need to manually specify a valid directory', 'backup-restore-manager' );
			/* OBZMOD */
			...
			*/
			$messages[] = sprintf(
				...
				__( 'Backups directory could not be created: (%1$s). You&apos;ll need to manually specify a valid directory or you can have WordPress do it automatically by entering your server details below. This is a one time thing.', 'backup-restore-manager' ),
				'<code>' . esc_html( Path::get_path() ) . '</code>'
			);
			/* OBZMOD */
		}
	...
	if ( ! is_readable( Path::get_root() ) ) {
		...
		$messages[] = sprintf(
			__( 'Your site&apos;s root path (%s) isn&apos;t readable. Please contact support.', 'backup-restore-manager' ),
	...
	if ( ! Requirement_Mysqldump_Command_Path::test() && ! Requirement_PDO::test() ) {
		...
		$messages[] = sprintf(
			...
			__( 'Your site cannot be backed up because your server doesn&apos;t support %1$s or %2$s. Please contact your host and ask them to enable them.', 'backup-restore-manager' ),
			'<code>mysqldump</code>',
			'<code>PDO::mysql</code>'
		);
	}
	...
	if ( disk_space_low() ) {
		...
		$messages[] = sprintf(
			__( 'Your server only has %s of disk space left which probably isn&apos;t enough to complete a backup. Try deleting some existing backups or other files to free up space.', 'backup-restore-manager' ),
...
/**
 * Hook in an change the plugin description when Backup & Restore Manager is activated
 *
 * @param array $plugins
...
function plugin_row( $plugins ) {
	...
	if ( isset( $plugins[ HMBKP_PLUGIN_SLUG . '/backup-restore-manager.php' ] ) ) {
		$plugins[ HMBKP_PLUGIN_SLUG . '/backup-restore-manager.php' ]['Description'] = str_replace( 'Once activated you\'ll find me under <strong>' . $menu . ' &rarr; Backups</strong>', 'Find me under <strong><a href="' . esc_url( get_settings_url() ) . '">' . $menu . ' &rarr; Backups</a></strong>', $plugins[ HMBKP_PLUGIN_SLUG . '/backup-restore-manager.php' ]['Description'] );
	}
...
function human_get_type( $type, Scheduled_Backup $schedule = null ) {
	...
	if ( strpos( $type, 'complete' ) !== false ) {
		return __( 'Database and Files', 'backup-restore-manager' );
	}

	if ( strpos( $type, 'file' ) !== false ) {
		return __( 'Files', 'backup-restore-manager' );
	}

	if ( strpos( $type, 'database' ) !== false ) {
		return __( 'Database', 'backup-restore-manager' );
	}
	...
	return __( 'Legacy', 'backup-restore-manager' );
...
function schedule_status( Scheduled_Backup $schedule, $echo = true ) {
	...
	<span class="hmbkp-status"<?php if ( $status->get_status() ) { ?> title="<?php printf( __( 'Started %s ago', 'backup-restore-manager' ), human_time_diff( $status->get_start_time() ) ); ?>"<?php } ?>>
		<?php echo $status->get_status() ? wp_kses_data( $status->get_status() ) : __( 'Starting backup...', 'backup-restore-manager' ); ?>
		<a href="<?php echo admin_action_url( 'request_cancel_backup', array( 'hmbkp_schedule_id' => $schedule->get_id() ) ); ?>"><?php _e( 'cancel', 'backup-restore-manager' ); ?></a>
	</span>
...
function backups_number( Scheduled_Backup $schedule ) {
	...
	if ( 0 === $number ) {
		$output = sprintf( __( 'No backups completed', 'backup-restore-manager' ) );
	} else {
		$output = sprintf( _nx( 'One backup completed', '%1$s backups completed', $number, 'backups count', 'backup-restore-manager' ), number_format_i18n( $number ) );
	}
...
function translated_schedule_title( $slug, $title ) {
	...
	$titles = array(
		'complete-hourly'      => esc_html__( 'Complete Hourly', 'backup-restore-manager' ),
		'file-hourly'          => esc_html__( 'File Hourly', 'backup-restore-manager' ),
		'database-hourly'      => esc_html__( 'Database Hourly', 'backup-restore-manager' ),
		'complete-twicedaily'  => esc_html__( 'Complete Twice Daily', 'backup-restore-manager' ),
		'file-twicedaily'      => esc_html__( 'File Twice Daily', 'backup-restore-manager' ),
		'database-twicedaily'  => esc_html__( 'Database Twice Daily', 'backup-restore-manager' ),
		'complete-daily'       => esc_html__( 'Complete Daily', 'backup-restore-manager' ),
		'file-daily'           => esc_html__( 'File Daily', 'backup-restore-manager' ),
		'database-daily'       => esc_html__( 'Database Daily', 'backup-restore-manager' ),
		'complete-weekly'      => esc_html__( 'Complete Weekly', 'backup-restore-manager' ),
		'file-weekly'          => esc_html__( 'File Weekly', 'backup-restore-manager' ),
		'database-weekly'      => esc_html__( 'Database Weekly', 'backup-restore-manager' ),
		'complete-fortnightly' => esc_html__( 'Complete Every Two Weeks', 'backup-restore-manager' ),
		'file-fortnightly'     => esc_html__( 'File Every Two Weeks', 'backup-restore-manager' ),
		'database-fortnightly' => esc_html__( 'Database Every Two Weeks', 'backup-restore-manager' ),
		'complete-monthly'     => esc_html__( 'Complete Monthly', 'backup-restore-manager' ),
		'file-monthly'         => esc_html__( 'File Monthly', 'backup-restore-manager' ),
		'database-monthly'     => esc_html__( 'Database Monthly', 'backup-restore-manager' ),
		'complete-manually'    => esc_html__( 'Complete Manually', 'backup-restore-manager' ),
		'file-manually'        => esc_html__( 'File Manually', 'backup-restore-manager' ),
		'database-manually'    => esc_html__( 'Database Manually', 'backup-restore-manager' ),
	);
