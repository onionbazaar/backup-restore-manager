...
<div class="hmbkp-exclude-settings">
	...
	<h3>
		<?php esc_html_e( 'Currently Excluded', 'backup-restore-manager' ); ?>
	</h3>
				...
				<tr>
					...
					<td>
						...
						<?php if (
							...
							<?php esc_html_e( 'Default rule', 'backup-restore-manager' ); ?>

						<?php elseif ( defined( 'HMBKP_EXCLUDE' ) && false !== strpos( HMBKP_EXCLUDE, $exclude ) ) : ?>

							<?php printf( esc_html__( 'Defined in %s', 'backup-restore-manager' ), 'wp-config.php' ); ?>
						...
						<?php else : ?>
							...
							<a href="<?php echo admin_action_url( 'remove_exclude_rule', array(
								...
								<?php esc_html_e( 'Stop excluding', 'backup-restore-manager' ); ?>
							</a>
	...
	<h3 id="directory-listing">
		<?php esc_html_e( 'Your Site', 'backup-restore-manager' ); ?>
	</h3>

	<p>
		<?php esc_html_e( 'Here\'s a directory listing of all files on your site, you can browse through and exclude files or folders that you don\'t want included in your backup.', 'backup-restore-manager' ); ?>
	</p>
	...
	<table class="widefat">
		...
		<thead>
			...
			<tr>
				<th></th>
				<th scope="col"><?php esc_html_e( 'Name', 'backup-restore-manager' ); ?></th>
				<th scope="col" class="column-format"><?php esc_html_e( 'Included Size', 'backup-restore-manager' ); ?></th>
				<th scope="col" class="column-format"><?php esc_html_e( 'Permissions', 'backup-restore-manager' ); ?></th>
				<th scope="col" class="column-format"><?php esc_html_e( 'Type', 'backup-restore-manager' ); ?></th>
				<th scope="col" class="column-format"><?php esc_html_e( 'Status', 'backup-restore-manager' ); ?></th>
			</tr>
					...
					<?php else :
						...
						?>
							...
							<code>
								...
								printf(
									esc_html__( '%1$s of %2$s', 'backup-restore-manager' ),
									$excluded_size,
									size_format( $size )
								);
								?>
								...
								<a class="dashicons dashicons-update" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'hmbkp_recalculate_directory_filesize',  urlencode( Path::get_root() ) ), 'hmbkp-recalculate_directory_filesize' ) ); ?>">
									<span><?php esc_html_e( 'Refresh', 'backup-restore-manager' ); ?></span>
								</a>
							</code>
				...
				<td>
					...
					<code>
						...
						<?php if ( is_link( Path::get_root() ) ) :
							esc_html_e( 'Symlink', 'backup-restore-manager' );
						elseif ( is_dir( Path::get_root() ) ) :
							esc_html_e( 'Folder', 'backup-restore-manager' );
						endif; ?>
				...
				<tr>
					...
					<td>
						...
						<?php if ( $file->isLink() ) : ?>
							<span title="<?php echo esc_attr( wp_normalize_path( $file->getRealPath() ) ); ?>">
								<?php esc_html_e( 'Symlink', 'backup-restore-manager' ); ?>
							</span>
						<?php elseif ( $file->isDir() ) :
							esc_html_e( 'Folder', 'backup-restore-manager' );
						else :
							esc_html_e( 'File', 'backup-restore-manager' );
						endif; ?>
						</code>
					</td>
					...
					<td class="column-format">
						...
						<?php if ( $is_unreadable ) : ?>
							...
							<strong title="<?php esc_attr_e( 'Unreadable files won\'t be backed up.', 'backup-restore-manager' ); ?>">
								<?php esc_html_e( 'Unreadable', 'backup-restore-manager' ); ?>
							</strong>

						<?php elseif ( $is_excluded ) : ?>

							<strong><?php esc_html_e( 'Excluded', 'backup-restore-manager' ); ?></strong>
						...
						<?php else :
							...
							<a href="<?php echo esc_url( wp_nonce_url(
								...
								<?php esc_html_e( 'Exclude &rarr;', 'backup-restore-manager' ); ?>
							</a>
		...
		<?php else : ?>
			...
			<tr>
				<td colspan="5">
					<span class="description"><?php esc_html_e( 'This folder is empty', 'backup-restore-manager' ); ?></span>
				</td>
			</tr>
	...
	<p class="submit">
		<a href="<?php echo esc_url( get_settings_url() ) ?>" class="button-primary">
			<?php esc_html_e( 'Done', 'backup-restore-manager' ); ?>
		</a>
	</p>
