...
<div class="wrap">
	<h1>
		<a class="page-title-action" href="<?php echo esc_url( get_settings_url() ); ?>"><?php esc_html_e( '&larr; Backups', 'backup-restore-manager' ); ?></a>
		<?php esc_html_e( 'BackUpWordPress Extensions', 'backup-restore-manager' ); ?>
	</h1>

	<div class="wp-filter">
		<p><?php //OBZMOD esc_html_e( 'Extend BackUpWordPress by installing extensions. Extensions allow you to pick and choose the exact features you need whilst also supporting us, the developers, so we can continue working on BackUpWordPress.', 'backupwordpress' ); ?></p>
	</div>
	...
	<h3><?php esc_html_e( 'Remote Storage', 'backup-restore-manager' ); ?></h3>

	<p><?php esc_html_e( 'It\'s important to store your backups somewhere other than on your site. Using the extensions below you can easily push your backups to one or more Cloud providers.', 'backup-restore-manager' ); ?></p>
				...
				<div class="plugin-card plugin-card-<?php echo esc_attr( $extension->slug ); ?>">
							...
							<ul class="plugin-action-buttons">
									...
									<li>
										...
										if (
											...
											<a
												...
												aria-label="<?php printf( esc_attr__( 'Update %s now', 'backup-restore-manager' ), esc_attr( $extension->title->rendered ) ); ?>"
												data-name="<?php esc_attr( $extension->title->rendered ); ?>">
												<?php esc_html_e( 'Update Now', 'backup-restore-manager' ); ?>
											</a>
										...
										elseif ( $is_extension_installed && $is_extension_active ) : ?>
											...
											<button
												...
												disabled="disabled">
												<?php echo esc_html_x( 'Active', 'Plugin status', 'backup-restore-manager' ); ?>
											</button>
										...
										elseif ( $is_extension_installed && ! $is_extension_active ) :
											...
											<a
												...
												aria-label="<?php printf( esc_attr__( 'Activate %s', 'backup-restore-manager' ), esc_attr( $extension->title->rendered ) ); ?>">
												<?php esc_html_e( 'Activate', 'backup-restore-manager' ); ?>
											</a>
										...
										// Buy Now - Not installed.
											...
											<a
												...
												aria-label="<?php printf( esc_attr__( 'Install %s now', 'backup-restore-manager' ), esc_attr( $extension->title->rendered ) ); ?>
												data-name="<?php echo esc_attr( $extension->title->rendered ); ?>">
												<?php printf( esc_html__( 'Buy Now &#36;%s', 'backup-restore-manager' ), esc_html( $extension->edd_price ) ); ?>
											</a>
								...
								<li>
									<a
										href="<?php echo esc_url( $extension->link ); ?>"
										class="thickbox"
										aria-label="<?php printf( esc_attr__( 'More information about %s', 'backup-restore-manager' ), esc_attr( $extension->title->rendered ) ) ; ?>"
										data-title="<?php echo esc_attr( $extension->title->rendered ); ?>">
										<?php esc_html_e( 'More Details', 'backup-restore-manager' ); ?>
									</a>
								</li>
							</ul>
						</div>
					...
					<div class="plugin-card-bottom" style="<?php echo esc_attr( $style ); ?>">
						...
						<div class="column-updated">
							<?php printf(
								wp_kses(
									__( '<strong>Last Updated:</strong> %s ago', 'backup-restore-manager' ),
									array(
										'strong' => array(),
									)
								),
